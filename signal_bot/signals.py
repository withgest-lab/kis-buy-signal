...
def bearish_divergence(close: pd.Series, indicator: pd.Series,
                       lookback: int = 30, min_gap: int = 5) -> pd.DataFrame:
    """RSI/MFI 하락 다이버전스(강세 다이버전스의 반대).

    오늘이 최근 `lookback`바 내 가격 최고점(신고가)이면서, 그 이전 고점(A) 대비
    indicator 값이 더 낮으면 감지. 신호는 신고가가 찍힌 당일에 발생한다.
    ...
    for t in range(lookback, n):
        window_start = t - lookback
        window = close.iloc[window_start:t + 1]
        pos_b = close.index.get_loc(window.idxmax())
        if pos_b != t:
            continue  # 오늘이 구간 내 최고점이 아니면 다이버전스 대상 아님

        early_end = pos_b - min_gap
        if early_end <= window_start:
            continue
        window_a = close.iloc[window_start:early_end + 1]
        pos_a = close.index.get_loc(window_a.idxmax())
        ...
        if price_b > price_a and ind_b < ind_a:
            detected.iloc[t] = True
            strength.iloc[t] = min(1.0, max(0.0, (ind_a - ind_b) / 20.0))
